 ...
 */
static void pf_pdport_load (pnet_t * net)
{
         ...
         sizeof (pnet_pdport_nvm_t)) == 0)
   {
      ...
      if (net->port[0].check.peer.length_peer_port_name > 0)
      {
...
static void pf_pdport_save (pnet_t * net)
{
...
int pf_pdport_read_ind (
   ...
   const pf_iod_read_result_t * read_result,
   uint16_t res_size,
   uint8_t * p_res,
   uint16_t * p_pos)
{
   ...
   case PF_IDX_DEV_PDREAL_DATA:
      if (
         (p_read_request->slot_number == PNET_SLOT_DAP_IDENT) &&
         ((p_read_request->subslot_number == PNET_SLOT_DAP_IDENT) ||
          (p_read_request->subslot_number == PNET_SUBMOD_DAP_IDENT)))
      {
 ...
 *          -1 if an error occurred.
 */
static int pf_cmwrr_pdport_data_adj (
   pnet_t * net,
   const pf_ar_t * p_ar,
   const pf_iod_write_request_t * p_write_request,
   uint8_t * p_bytes, /* Not const as it is used in a pf_get_info_t */
