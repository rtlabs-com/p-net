 ...
 * @param udp_port   In: Upd protocol port
 */
   ...
   uint16_t udp_port)
{
   ...
   if (epm_type == PF_EPM_TYPE_EPMV4)
   {
      ...
      p_tower->floor_1.version_major = RPC_FLOOR_VERSION_EPMv4;
   }
   else
   {
      memcpy (
         ...
         sizeof (p_tower->floor_1.uuid));
      p_tower->floor_1.version_major = RPC_FLOOR_VERSION_PNIO;
   }
   p_tower->floor_1.version_minor = RPC_FLOOR_VERSION_MINOR;

   /* Floor 2 32bit NDR*/
   p_tower->floor_2.protocol_id = PF_RPC_PROTOCOL_UUID;
   memcpy (&p_tower->floor_2.uuid, &uuid_ndr, sizeof (p_tower->floor_2.uuid));
   p_tower->floor_2.version_major = RPC_FLOOR_VERSION_NPR;
   p_tower->floor_2.version_minor = RPC_FLOOR_VERSION_MINOR;

   /* Floor 3 RPC*/
   p_tower->floor_3.protocol_id = PF_RPC_PROTOCOL_CONNECTIONLESS;
   p_tower->floor_3.version_minor = RPC_FLOOR_VERSION_MINOR;
...
int pf_cmrpc_lookup_request (
   pnet_t * net,
   const pf_rpc_header_t * p_rpc_req,
   pf_rpc_lookup_req_t * p_lookup_req,
   pnet_result_t * p_read_status,
   uint16_t res_size,
   uint8_t * p_res,
   uint16_t * p_pos)
