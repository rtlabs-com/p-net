...
static pf_cmsu_state_values_t pf_cmsu_state = PF_CMSU_STATE_IDLE;
   ...
   case PF_CMSU_STATE_IDLE:
      ...
      for (crep = 0; crep < p_ar->nbr_iocrs; crep++)
      {
         ...
         if (ret == 0)
         {
            if (p_ar->iocrs[crep].param.iocr_type == PF_IOCR_TYPE_INPUT)
            {
               if (pf_ppm_activate_req(p_ar, crep) != 0)
               {
                  p_stat->pnio_status.error_code_2 = PNET_ERROR_CODE_2_CMSU_AR_ADD_PROV_CONS_FAILED;
                  ret = -1;
               }
      ...
      if ((ret == 0) && (pf_alarm_activate(p_ar) != 0))
      {
      ...
      pf_cmsu_set_state(PF_CMSU_STATE_STARTUP);
      /* CMSU_Start.cnf */
      break;
   case PF_CMSU_STATE_STARTUP:
   ...
   uint8_t                 err_code)
{
   ...
   return pf_cmdev_state_ind(p_ar, PNET_EVENT_ABORT);
}
