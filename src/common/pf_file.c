...
#define PF_FILE_VERSION    0x00000001U   /* Increase every time the saved contents have another format */
...
)
{
   ...
   {
      LOG_ERROR(PNET_LOG, "FILE(%d): Filename and outputbuffer must be given!\n", __LINE__);
      return -1;
   }
   ...
   if (use_directory == true)
   {
      ...
      {
         LOG_ERROR(PNET_LOG, "FILE(%d): Too long directory and filename! Max total length %zu\n", __LINE__, size);
         return -1;
      }
   ...
   else
   {
      ...
      {
         LOG_ERROR(PNET_LOG, "FILE(%d): Too long filename! Max total length %zu\n", __LINE__, size);
         return -1;
      }
...
)
{
   ...
   if (pf_file_join_directory_filename(directory, filename, path, PNET_MAX_FILE_FULLPATH_LEN) != 0)
   {
      return -1;
   }

   /* Read file */
   if(os_load_file(path, versioning_buffer, sizeof(versioning_buffer), object, size) != 0)
   {
      return -1;
   }
   ...
   {
      LOG_ERROR(PNET_LOG, "FILE(%d): Wrong file magic bytes in file %s\n",
         __LINE__, path);
      return -1;
   }
   ...
   {
      LOG_WARNING(PNET_LOG, "FILE(%d): Wrong file version identifier in file %s  Expected %" PRIu32 " but got %" PRIu32 ".\n",
         __LINE__, path, (uint32_t)PF_FILE_VERSION, version);
      return -1;
   }
...
)
{
   ...
   pf_put_uint32(true, PF_FILE_MAGIC, sizeof(versioning_buffer), versioning_buffer, &pos);  /* Big endian */
   pf_put_uint32(true, PF_FILE_VERSION, sizeof(versioning_buffer), versioning_buffer, &pos);  /* Big endian */

   return os_save_file(path, versioning_buffer, sizeof(versioning_buffer), object, size);
}
