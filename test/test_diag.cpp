...
TEST_F (DiagTest, DiagRunTest)
{
   ...
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0001, 0x0002, 0x00030004, 0, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, 0);

   ret = pnet_diag_update(g_pnet, main_arep, 0, 1, 1, 0, ch_properties,
      0x0001,     /* ch_error_type */
      0x00030004, /* add_value */
      PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, 0);

   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0001, PNET_DIAG_USI_STD);
   EXPECT_EQ(ret, 0);
   ...
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0002, 0x0002, 0x00030004, 0, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, 0);

   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_DEMANDED);
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0003, 0x0002, 0x00030004, 0, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, 0);

   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_QUALIFIED);
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0004, 0x0002, 0x00030004, 0x00010000, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, 0);

   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_QUALIFIED);
   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0005, PNET_DIAG_USI_STD);
   EXPECT_EQ(ret, -1);
   ...
   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_REQUIRED);
   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0002, PNET_DIAG_USI_STD);
   EXPECT_EQ(ret, 0);

   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_DEMANDED);
   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0003, PNET_DIAG_USI_STD);
   EXPECT_EQ(ret, 0);

   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_QUALIFIED);
   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0004, PNET_DIAG_USI_STD);
   EXPECT_EQ(ret, 0);
   ...
   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_REQUIRED);
   ret = pnet_diag_update(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0007, 0x00030004, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, -1);

   /* Add diag to non-existing sub-slot */
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 2, 0, ch_properties, 0x0002, 0x0002, 0x00030004, 0, PNET_DIAG_USI_STD, NULL);
   EXPECT_EQ(ret, -1);

   /* Test with a different USI */
   PNET_DIAG_CH_PROP_MAINT_SET(ch_properties, PNET_DIAG_CH_PROP_MAINT_REQUIRED);
   ret = pnet_diag_add(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0002, 0x0002, 0x00030004, 0, 0x1234, (uint8_t*)"Bjarne");
   EXPECT_EQ(ret, 0);

   /* Remove wrong USI */
   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0002, 0x1235);
   EXPECT_EQ(ret, -1);

   ret = pnet_diag_remove(g_pnet, main_arep, 0, 1, 1, 0, ch_properties, 0x0002, 0x1234);
   EXPECT_EQ(ret, 0);
